...
static void
LanceApplyGlobalSearchSplits(optional_ptr<FunctionData> bind_data,
                             const vector<DistributedScanSplit> &splits) {
  auto *split_state =
      dynamic_cast<LanceGlobalSearchSplitState *>(bind_data.get());
  if (!split_state) {
...
static unique_ptr<FunctionData> LanceCreateDistributedKnnWorkerBind(
  ...
  if (source.namespace_backed) {
    throw NotImplementedException(
        "Distributed Lance REST namespace vector search is unavailable "
        "because query_table does not expose a stable dataset generation");
  }
  if (source.non_replayable_storage_options) {
    throw NotImplementedException(
        "Distributed Lance vector search requires credential-free storage "
...
static void LanceKnnSerialize(Serializer &serializer,
  ...
  if (targets_distributed_workers && bind_data.namespace_backed) {
    throw SerializationException(
        "REST namespace vector search cannot use distributed plan transport "
        "because query_table lacks a stable dataset generation");
  }
  if (targets_distributed_workers && bind_data.non_replayable_storage_options) {
    throw SerializationException(
        "Lance vector-search storage credentials or URI parameters cannot be "
...
#ifdef LANCE_VANE_DISTRIBUTED
  serializer.WriteProperty(126, "distributed_worker_bind",
...
static unique_ptr<FunctionData> LanceKnnDeserialize(Deserializer &deserializer,
  ...
  result->distributed_worker_bind =
      deserializer.ReadProperty<bool>(126, "distributed_worker_bind");
#endif
  ...
  ValidateSerializedSearchCString(result->dataset_generation_id,
                                  "dataset generation");
  ...
  if (result->namespace_backed) {
    ...
    if (result->distributed_worker_bind) {
      throw SerializationException(
          "Distributed Lance REST namespace vector search cannot validate a "
...
static unique_ptr<FunctionData> LanceCreateDistributedSearchWorkerBind(
  ...
  if (source.namespace_backed) {
    throw NotImplementedException(
        "Distributed Lance REST namespace search is unavailable because "
        "query_table does not expose a stable dataset generation");
  }
  if (source.non_replayable_storage_options) {
    throw NotImplementedException(
        "Distributed Lance search requires credential-free storage options "
        "and a worker-local credential chain; run locally when credentials "
        "must be carried by the connection");
  }
  auto result = make_uniq<LanceSearchBindData>();
...
static void LanceSearchSerialize(Serializer &serializer,
  ...
  if (targets_distributed_workers && bind_data.namespace_backed) {
    throw SerializationException(
        "REST namespace search cannot use distributed plan transport because "
        "query_table lacks a stable dataset generation");
  }
  if (targets_distributed_workers && bind_data.non_replayable_storage_options) {
    throw SerializationException(
        "Lance search storage credentials or URI parameters cannot be "
        "serialized to workers");
  }
  ...
  serializer.WriteProperty(130, "non_replayable_storage_options",
                           bind_data.non_replayable_storage_options);
#ifdef LANCE_VANE_DISTRIBUTED
  serializer.WriteProperty(131, "distributed_worker_bind",
                           bind_data.distributed_worker_bind);
#endif
}
...
LanceSearchDeserialize(Deserializer &deserializer, TableFunction &) {
...
#ifdef LANCE_VANE_DISTRIBUTED
  result->distributed_worker_bind =
      deserializer.ReadProperty<bool>(131, "distributed_worker_bind");
#endif
  if (result->k == 0) {
  ...
  if (result->namespace_backed) {
    ...
    if (result->distributed_worker_bind) {
      throw SerializationException(
          "Distributed Lance REST namespace search cannot validate a stable "
          "dataset generation");
    }
#endif
  } else {
